"""
In this experiment, we are going to use all (day 6 & terminal state) cells to be training set
"""
import sys
...
from classifier import train_and_predict_rf, store_variable_importance
from classifier import store_prediction_sets, store_prediction_sizes, store_test_probabilities
...
pseudo_time_ranks = np.argsort(data.obs['palantir_pseudotime'][cells2recruit_barcode].values)[::-1]

dummy_labels = pd.get_dummies(data[day6_terminal_barcode].obs['state_info']).astype('int')
# Get the mapping between variables in state_info and numbers
state_info_mapping = {state: idx for idx, state in enumerate(dummy_labels.columns)}
print("Mapping between state_info and numbers:", state_info_mapping)
...
F.loc[day6_terminal_barcode] = dummy_labels.values
data.obsm['dummy_label'] = F

data.obs['visit'] = [1] * data.n_obs
data.obs.loc[~data.obs_names.isin(day6_terminal_barcode), 'visit'] = 0
...
data.obs.loc[day6_terminal_barcode, 'iteration_recruited'] = -1

labeled_cells_barcode = data.obs_names[data.obs['visit'] == 1].tolist()
num_unlabeled_total = data[data.obs['visit'] != 1].shape[0]

#build the graph
import scipy.sparse as sp
#graph = kevin_graph(data.obsm['X_pca'], 10, metric="euclidean") 
#sp.save_npz("/home/yzhao4/new_repo_branchpoint/Output/label_propagation_clamping/Larry/sparse_matrix.npz", graph)
graph = sp.load_npz("/home/yzhao4/new_repo_branchpoint/Output/label_propagation_clamping/Larry/sparse_matrix.npz")
...
while np.sum(data.obs['visit']) < data.n_obs:
    ...
    test_ind = data[cells2recruit_barcode][pseudo_time_ranks[
    ...
    cal_ind = train_pseudotime_sorted.index[:recruitment_size].tolist()
    train_ind = train_pseudotime_sorted.index[recruitment_size:].tolist()

    X_train = data[train_ind].X.toarray().copy()
    X_test = data[test_ind].X.toarray().copy()
    X_cal = data[cal_ind].X.toarray().copy()
    ...
    #Fm = truth_label_clamping(graph, Fm, F_true, iter_max=10)
    ...
    #store_iteration_probabilities(data, prob_test_prop, test_ind, k)
    ...
    # Store sizes and prediction sets in conformal_result
    ...
    predicted_label = np.zeros((len(test_ind), 8))
