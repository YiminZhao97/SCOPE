...
parser = argparse.ArgumentParser(description='branchpoint prediction with covariate shift')
# Data and I/O information
parser.add_argument('--input_data', type=str, default='/home/yzhao4/new_repo_branchpoint/Data/Simulation/Principle_curve/sim_different_skeleton/sim1/simulation_processed_data_sim1.h5ad',
                    help='Path to input h5ad file')
parser.add_argument('--output_dir', type=str, default='/home/yzhao4/new_repo_branchpoint/Output/when_sim_fails/paper_res/sim1',
                    help='Output directory for results')
parser.add_argument('--omit_tail', type=float, default=0,
                    help='Omit tail parameter for prediction set')
parser.add_argument('--alpha', type=float, default=0.1,
                    help='Alpha level for conformal prediction')
...
parser.add_argument('--iter_graph', type=int, default=100,
                    help='Maximum iterations for label spreading')
parser.add_argument('--initial_trees', type=int, default=100,
                    help='Initial number of trees for random forest')
parser.add_argument('--trees_per_iteration', type=int, default=10,
                    help='Trees to add per iteration')
parser.add_argument('--n_neighbors', type=int, default=10,
                    help='Number of neighbors for graph construction')
parser.add_argument('--cal_multiplier', type=int, default=2,
                    help='Multiplier for calibration set size (cal_size = recruitment_size * cal_multiplier)')
...
#pseudo time check
pseudo_time_ranks = np.argsort(data.obs['pseudotime'][data.obs['cluster'] == 'TBD'].values)  #np.argsort()
pseudo_time_ranks = pseudo_time_ranks[::-1]

# Create dummy labels for the 3 classes in original data
dummy_labels = pd.get_dummies(data[data.obs['cluster'] != 'TBD'].obs['cluster']).astype('int')
# Get the mapping between variables in cluster and numbers
state_info_mapping = {state: idx for idx, state in enumerate(dummy_labels.columns)}
print("Mapping between cluster and numbers:", state_info_mapping)
...
F.loc[data.obs_names[data.obs['cluster'] != 'TBD']] = dummy_labels.values
data.obsm['dummy_label'] = F

data.obs['visit'] = [1] * data.n_obs
data.obs.loc[data.obs_names[data.obs['cluster'] == 'TBD'], 'visit'] = 0
...
data.obs.loc[data.obs_names[data.obs['cluster'] != 'TBD'], 'iteration_recruited'] = -1

labeled_cells_barcode = data.obs_names[data.obs['visit'] == 1].tolist()
num_unlabeled_total = data[data.obs['cluster'] == 'TBD'].shape[0]

# Build the graph
graph = kevin_graph(data.obsm['X_pca'], args.n_neighbors, metric="euclidean")
...
cells2recruit_barcode = data.obs_names[data.obs['cluster'] == 'TBD'].tolist()
...
while np.sum(data.obs['visit']) < data.n_obs:
    ...
    test_ind = data[data.obs['cluster'] == 'TBD'][pseudo_time_ranks[
    ...
    train_pseudotime = data[labeled_cells_barcode].obs['pseudotime'].copy(deep=True)
    ...
    cal_ind = train_pseudotime_sorted.index[:args.cal_multiplier * recruitment_size].tolist()
    train_ind = train_pseudotime_sorted.index[args.cal_multiplier * recruitment_size:].tolist()
    ...
    #store_variable_importance(data, trained_classifiers, k, available_types, state_info_mapping)
    ...
    Fm = label_spreading(graph, Fm, alpha = args.label_spreading_alpha, iter_max=args.iter_graph)
    ...
    qhat = find_qhat(df_score_weight['score'].tolist(), df_score_weight['weight'].tolist(), alpha = args.alpha)
