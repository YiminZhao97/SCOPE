"""
In this experiment, we are going to use all cells to be training set following the warmstart logic
"""
import sys
...
# Process data similar to warmstart version but adapted for Palantir data
pseudo_time_ranks = np.argsort(data.obs['palantir_pseudotime'][data.obs['cluster'] == 'TBD'].values)[::-1]
...
# Build the graph
graph = kevin_graph(data.obsm['vae_latent_space'], 10, metric="euclidean") 
...
while np.sum(data.obs['visit']) < data.n_obs:
    ...
    X_train = data[train_ind].obsm['imputed_hvg'].copy()
    X_test = data[test_ind].obsm['imputed_hvg'].copy()
    X_cal = data[cal_ind].obsm['imputed_hvg'].copy()
